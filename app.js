...
function editGroup(name, items) { $('#editId').value=`group:${name}`; $('#editName').value=name; $('#editOverlay').checked=items.some(item => item.overlay); fillScheduleSettings(items.find(item => item.scheduleEnabled) || items[0]); ui.dialog.showModal(); }
